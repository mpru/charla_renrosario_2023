...
#' Definición de la función interna (queda propuesto: documentarla)
#' @keywords internal
#' 
.bisiesto <- function(x, idioma) {
  ...
  mensaje <- paste(x, ifelse(res, mensajes[[idioma]]$es_bisiesto, mensajes[[idioma]]$no_es_bisiesto), collapse = "\n")
  cat(mensaje)
}
```
...
- A la lista global `mensajes` le agregamos una sublista llamada `"es"`, con los tres mensajes que emite nuestro paquete, escritos en español:
...
- Creamos un archivo para guardar todos los *wrapers* en Español que llaman a las funciones internas:

```{r, eval=FALSE}
use_r("funciones_es.R")
```

- Implementamos el *wraper*, usando el argumento `idioma = "es"`.
...
- Tanto los nombres de las funciones, como los mensajes emitidos y las páginas de ayuda están disponibles en los lenguajes implementados^[Para ver las páginas de ayuda, tenemos que completar las documentaciones que dejamos pendiente y hacer una instalación de nuestro paquete en desarrollo con `install()`].
